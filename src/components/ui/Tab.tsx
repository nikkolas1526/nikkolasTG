...
const Tab: FC<OwnProps> = ({
  ...
  useEffect(() => {
    if (typeof title == "string") {
      ...
      var reset = true;
      title.map((item) => {
        if (typeof item === "object" && item.type === 2) {
          reset = false;
          return null;
        } else {
          return null;
  ...
  const Regex =
    /[\u{1F600}-\u{1F64F}\u{1F300}-\u{1F5FF}\u{1F680}-\u{1F6FF}\u{1F700}-\u{1F77F}\u{1F780}-\u{1F7FF}\u{1F800}-\u{1F8FF}\u{1F900}-\u{1F9FF}\u{1FA00}-\u{1FA6F}\u{1FB00}-\u{1FBFF}\u{2600}-\u{26FF}\u{2700}-\u{27BF}]/u;
    ...
    return (
      <>
        {title.map((item, index) => {
          ...
          if (typeof item === "object" && item.type === 2 && !set2) {
            const { src, alt, className } = item.props;
            setSet(true);
            set2 = true;
            switch (alt) {
              case "💬":
        ...
        <span className="Tab_inner">
          {typeof title === "string"
            ? renderText(title.replace(Regex, ""))
            : title.map((item, index) => {
  ...
  return (
    ...
    >
      {windowWidth > 1000 ? (
        ...
        <span className="Tab_inner">
          {typeof title === "string" ? renderText(title.trim()) : title}
          {Boolean(badgeCount) && (
            <span
              className={buildClassName(
                "badge",
